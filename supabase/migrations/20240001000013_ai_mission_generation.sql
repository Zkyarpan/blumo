-- Unit 10: authenticated AI mission generation.
-- Claims one task per user-local day, preserves history, and keeps all privileged
-- generation lifecycle writes behind service-role-only transactional functions.

alter table public.daily_tasks
  add column difficulty text,
  add column suggested_branch text,
...
alter table public.daily_tasks
  add constraint daily_tasks_generation_attempts_check
  check (generation_attempts > 0),
  add constraint daily_tasks_unit10_fields_check
  check (
...
-- This intentionally fails rather than deleting or merging history if legacy
-- duplicates exist. Operators must resolve such data explicitly before applying.
create unique index daily_tasks_one_per_user_date_idx
  on public.daily_tasks (user_id, scheduled_date);

-- An unfinished mission remains the user's one active mission even when its
-- scheduled date is earlier. This intentionally fails on pre-existing active
-- duplicates so operators must resolve history explicitly.
...
alter table public.ai_usage_records
  add column provider_call_id uuid;

create unique index ai_usage_records_provider_call_id_idx
  on public.ai_usage_records (provider_call_id)
  where provider_call_id is not null;

-- Mission lifecycle writes must pass through the authenticated server action and
-- the transactional service-role functions below. Broad browser writes could
-- otherwise bypass the one-per-day claim or overwrite completed history.
drop policy if exists "daily_tasks: owner insert" on public.daily_tasks;
drop policy if exists "daily_tasks: owner update" on public.daily_tasks;
...
begin
  ...
  -- still unfinished. Completed, failed, and archived rows remain history.
